...
function getFastestResponse(delays) {
    // Створюємо масив промісів на основі масиву затримок
...
function withTimeout(promise, ms) {
    // Створюємо проміс, який гарантовано впаде з помилкою через ms мілісекунд
    const timeout = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Timeout')), ms);
    });

    // Хто швидше: оригінальний проміс чи наш таймер-вбивця?
...
function fetchFromFastestServer() {
    // Створюємо три запити (вони починають виконуватися одразу)
