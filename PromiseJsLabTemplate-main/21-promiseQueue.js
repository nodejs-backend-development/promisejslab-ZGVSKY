...
class PromiseQueue {
    ...
    add(promiseFactory) {
        // Додаємо функцію, яка створює проміс, у масив
        this.queue.push(promiseFactory);
        
        // Якщо черга зараз стоїть, запускаємо її
        if (!this.processing) {
            this.process();
        }
    }
    ...
    async process() {
        // Якщо черга порожня, зупиняємо обробку
        ...
        // Беремо перше завдання з початку масиву
        const nextTask = this.queue.shift();
        
        try {
            // Виконуємо його і чекаємо на завершення
            await nextTask();
        } catch (error) {
            console.error('Task failed:', error);
        } finally {
            // Незалежно від успіху чи помилки, рекурсивно запускаємо наступне завдання
            this.process();
        }
    }
}
...
class PriorityQueue {
    ...
    add(promiseFactory, priority = 0) {
        // Зберігаємо і саму функцію, і її пріоритет
        this.queue.push({ factory: promiseFactory, priority: priority });
        
        // Сортуємо масив за спаданням пріоритету
    ...
    async process() {
        ...
        // shift() тепер завжди братиме завдання з найвищим пріоритетом (через сортування вище)
        const nextTask = this.queue.shift();
        
        try {
            // Зверни увагу, що ми викликаємо .factory()
            await nextTask.factory();
        } catch (error) {
            console.error('Task failed:', error);
        } finally {
            this.process();
